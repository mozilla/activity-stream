...
class BlockedURLs extends Set {
   ...
   * @returns {Integer} number of items added
   */
  save(url) {
    let oldSize = this.size;
    this.add(url);
    let sizeDiff = this.size - oldSize;
    if (sizeDiff > 0) {
  ...
  remove(url) {
    let oldSize = this.size;
    this.delete(url);
    let sizeDiff = oldSize - this.size;
    if (sizeDiff > 0) {
      simplePrefs.prefs[this._prefName] = JSON.stringify(this.items());
    }
    return sizeDiff;
...
Links.prototype = {
   ...
   */
  blockURL(url) {
    if (this.blockedURLs.save(url) > 0) {
   ...
   */
  unblockURL(url) {
    if (this.blockedURLs.remove(url) > 0) {
