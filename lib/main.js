...
const am = require("src/actions/action-manager");
const {actions} = am;
...
ActivityStreams.prototype = {
    ...
    switch(msg.type) {
      case am.type("TOP_FRECENT_SITES_REQUEST"):
        PlacesProvider.links.getTopFrecentSites(msg.data).then(links => {
          this.send(actions.Response("TOP_FRECENT_SITES_RESPONSE", links), worker);
        });
        break;
    }
  },
  ...
  _handlePlacesChanges(eventName, data) {
    this.broadcast(actions.Response("RECEIVE_PLACES_CHANGES", {
      "event": eventName,
      "data": data
    }));
  },
