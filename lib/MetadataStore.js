  ...
  */
  ...
  _waitForAsyncReset() {
    return waitUntil(function*() {
      ...
      try {
        let nMetadata = yield this.asyncExecuteQuery(
          "SELECT count(*) as count FROM page_metadata",
          {"columns": ["count"]});
        let nImages = yield this.asyncExecuteQuery(
          "SELECT count(*) as count FROM page_images",
          {"columns": ["count"]});
        let nMetadataImages = yield this.asyncExecuteQuery(
   ...
   * database connection
   */
  asyncDelete: Task.async(function*() {
    try {
