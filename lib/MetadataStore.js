...
const SQL_INSERT_METADATA = `INSERT INTO page_metadata
  (cache_key, canonical_url, title, type, description, media_url, expired_at)
  VALUES
  (:cache_key, :canonical_url, :title, :type, :description, :media_url, :expired_at)`;
...
MetadataStore.prototype = {
  ...
  _createTableSchema() {
    return Task.spawn(function*() {
      try {
        this._conn.executeTransaction(function*() {
  ...
  _getMetadataParameters(aRow) {
    return {
      ...
      canonical_url: aRow.canonical_url,
      title: aRow.title,
      type: aRow.type || null,
  ...
  */
   ...
   *
   * aRows, an array of metadata objects that currently generated by embed.ly
   * Returns a promise that is resolved upon success, or rejected if an exception occurs
   */
  asyncInsert: Task.async(function*(aRows) {
    ...
    for (let row of aRows) {
      yield this._conn.executeTransaction(function*() {
        ...
        /* attach favicon to canonical_url for Places */
        PlacesUtils.favicons.setAndFetchFaviconForPage(
          NetUtil.newURI(row.canonical_url),
          NetUtil.newURI(row.favicon_url),
          false,
        ...
        try {
          let metadataBindings = this._getMetadataParameters(row);
          yield this._conn.executeCached(SQL_INSERT_METADATA, metadataBindings);
          metadata_id = yield this._getLastInsertRowID();
        } catch (e) {
        ...
        let faviconBindings = this._getFaviconParameters(row);
        try {
          yield this._conn.executeCached(SQL_INSERT_IMAGES, faviconBindings);
          image_ids.push(yield this._getLastInsertRowID());
        } catch (e) {
          try {
            image_ids.push(yield this._getImageIDByURL(faviconBindings.url));
          } catch (e) {
        ...
        for (let image of row.images) {
          ...
          try {
            ...
            try {
              image_ids.push(yield this._getImageIDByURL(imageBindings.url));
            } catch (e) {
  ...
  /**
  * Enables the data expiry cron. The database connection needs to
  * be established prior to calling this function. Once the cron gets
  * triggered, following calls will be ignored unless the user disables
  * it by disableDataExpiryCron()
  *
  * @param {Number} interval
  *        an time interval in millisecond for this cron
  */
  enableDataExpiryCron(interval) {
    if (!this._conn) {
