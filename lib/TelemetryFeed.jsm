...
this.TelemetryFeed = class TelemetryFeed {
  constructor(options) {
    ...
    this.structuredIngestionTelemetryEnabled = this._prefs.get(STRUCTURED_INGESTION_TELEMETRY_PREF);
    this._structuredIngestionEndpointBase = this._prefs.get(STRUCTURED_INGESTION_ENDPOINT_PREF);
    ...
    this._onStructuredIngestionTelemetryPrefChange = this._onStructuredIngestionTelemetryPrefChange.bind(this);
    this._prefs.observe(STRUCTURED_INGESTION_TELEMETRY_PREF, this._onStructuredIngestionTelemetryPrefChange);
  ...
  _onStructuredIngestionTelemetryPrefChange(prefVal) {
    this.structuredIngestionTelemetryEnabled = prefVal;
  }
  ...
  _generateStructuredIngestionEndpoint(pingType, version) {
    ...
    const docID = uuid.substring(1, uuid.length - 1);
    const extension = `${pingType}/${version}/${docID}`;
    return `${this._structuredIngestionEndpointBase}/${extension}`;
  }
  ...
  uninit() {
    ...
    try {
      ...
      this._prefs.ignore(STRUCTURED_INGESTION_TELEMETRY_PREF, this._onStructuredIngestionTelemetryPrefChange);
    } catch (e) {
      Cu.reportError(e);
    }
