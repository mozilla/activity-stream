/* globals require, exports, XPCOMUtils, CustomizableUI, Services, Social */
...
// This is to register a ShareUtils object in the browser chrome window
const Overlay = {
  init: eventTracker => {
...
ShareProvider.prototype = {
  ...
  _setupProviders() {
    if (!Services.prefs.prefHasUserValue("social.activeProviders")) {
      for (let key of Object.keys(DEFAULT_MANIFEST_PREFS)) {
        ...
        SocialService.enableProvider(DEFAULT_MANIFEST_PREFS[key].origin, () => {});
      }
  ...
  _unsetupProviders() {
    if (Services.prefs.getBoolPref("social.enabledByActivityStream")) {
      for (let key of Object.keys(DEFAULT_MANIFEST_PREFS)) {
        SocialService.uninstallProvider(DEFAULT_MANIFEST_PREFS[key].origin, () => {
