...
ActivityStreams.prototype = {
  ...
  /**
   * Responds to bookmarks requests
   */
  _respondToBookmarksRequests(msgName, params) {
    const {worker} = params;
    // This is just a quick and dirty hack for now,
    // it does not handle changes
    Bookmarks.search({sort: "updated", descending: true}).on("end", results => {
      this.send(am.actions.Response("RECENT_BOOKMARKS_RESPONSE", results), worker);
    });
  },
  ...
  _setupListeners() {
    ...
    this.on(am.type("RECENT_BOOKMARKS_REQUEST"), this._respondToBookmarksRequests.bind(this));
  },
  ...
  _removeListeners() {
    ...
    this.on(am.type("RECENT_BOOKMARKS_REQUEST"), this._respondToBookmarksRequests);
  },
