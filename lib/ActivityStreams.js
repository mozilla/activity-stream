...
function ActivityStreams(options = {}) {
  ...
  this._asyncGetRecommendations();
  this._startRecommendationRefresh(this.options.recommendationTTL);
}
...
ActivityStreams.prototype = {
  ...
  _processAndSendLinks(placesLinks, responseType, worker, options) {
    let {append, previewsOnly, skipPreviewRequest, getRecommendation} = options || {};
    ...
    let recommendation = this._recommendationProvider.getARecommendation(getRecommendation && simplePrefs.prefs.recommendations && inExperiment);
  ...
  /**
    * Start a timer to fetch a new recommendation every hour
    */
  _startRecommendationRefresh(recommendationTTL) {
    if (recommendationTTL) {
      this._newRecommendationTimeoutID = setTimeout(() => {
        if (simplePrefs.prefs.recommendations) {
          this._asyncGetNewRecommendation();
        }
        this._startRecommendationRefresh(recommendationTTL);
      }, recommendationTTL);
    }
  },

  /**
    * Every hour we want to get a new recommendation
    */
  _asyncGetNewRecommendation: Task.async(function*() {
    this._recommendationProvider.getANewRecommendation(true);
  }),

  /**
    * Fetch the recommendations
    */
  _asyncGetRecommendations: Task.async(function*() {
    yield this._recommendationProvider.asyncSetRecommendedContent();
  }),
