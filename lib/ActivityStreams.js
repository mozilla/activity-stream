...
function ActivityStreams(metadataStore, options = {}) {
  ...
  // This is instantiated with the recommender based on weights if pref is true.
...
ActivityStreams.prototype = {
  ...
  _loadRecommender() {
    console.log("initialize the recommender");
    // Only need to load history items once per session, any changes are very relevant to the scoring function.
    this._memoized.getAllHistoryItems().then(historyItems => {
      let highlightsCoefficients = JSON.parse(simplePrefs.prefs.weightedHighlightsCoefficients);
      this.baselineRecommender = new Recommender(historyItems, {highlightsCoefficients});
    });
  },
  ...
  _respondToPlacesRequests({msg, worker}) {
    ...
    switch (msg.type) {
      case am.type("WEIGHTED_HIGHLIGHTS_REQUEST"):
        ...
        if (this.baselineRecommender === null) { //simplePrefs.prefs.weightedHighlights || this.baselineRecommender === null) {
        ...
        provider.getHighlightsLinks(msg.data).then(highlightsLinks => {
          ...
          cachedLinks.then(highlightsWithMeta => {
            this.send(am.actions.Response("WEIGHTED_HIGHLIGHTS_RESPONSE", this.baselineRecommender.scoreEntries(highlightsWithMeta),
  ...
  _setupListeners() {
    ...
    simplePrefs.on("", prefName => {
      if (prefName === "weightedHighlightsCoefficients") {
        this.baselineRecommender.updateOptions({highlightsCoefficients: JSON.parse(simplePrefs.prefs.weightedHighlightsCoefficients)});
      }
