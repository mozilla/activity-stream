...
ActivityStreams.prototype = {
  ...
  _respondToExperimentsRequest(msgName, params) {
    let {worker} = params;
  ...
  _setupListeners() {
    ...
    this._respondToExperimentsRequest = this._respondToExperimentsRequest.bind(this);
    ...
    this.on(am.type("EXPERIMENTS_REQUEST"), this._respondToExperimentsRequest);
  },
  ...
  _removeListeners() {
    ...
    this.off(am.type("EXPERIMENTS_REQUEST"), this._respondToExperimentsRequest);
  },
   ...
   */
  _setHomePage() {
    // Only hijack the home page if it is set to about:home or about:blank
    if (["about:home", "about:blank"].includes(prefService.get(HOME_PAGE_PREF))) {
