...
ActivityStreams.prototype = {
  ...
  _respondToPlacesRequests({msg, worker}) {
    ...
    switch (msg.type) {
      case am.type("WEIGHTED_HIGHLIGHTS_REQUEST"):
        // Empty response in case the pref is off, no need for the extra requests.
        ...
        Promise.all([
          provider.getAllHistoryItems(msg.data), // Count all the bookmarks.
          provider.getHighlightsLinks(msg.data) // Get all recent links.
        ]).then(value => {
          // Decorate links with meta information.
          let cachedLinks = this._processLinks(value[1], "WEIGHTED_HIGHLIGHTS_RESPONSE", msg.meta);
          cachedLinks.then(highlightsWithMeta => {
            this.send(am.actions.Response("WEIGHTED_HIGHLIGHTS_RESPONSE", Recommender(highlightsWithMeta, value[0]),
  ...
  _respondToPrefsRequest() {
    this.broadcast(am.actions.Response("PREFS_RESPONSE", {
      recommendationStatus: simplePrefs.prefs.recommendations,
      weightedHighlights: simplePrefs.prefs.weightedHighlights
    }));
  },
  ...
  _respondToUIChanges(args) {
    ...
    switch (msg.type) {
      ...
      case am.type("PREFS_REQUEST"):
        return this._respondToPrefsRequest();
