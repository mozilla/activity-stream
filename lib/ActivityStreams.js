    ...
    switch (msgName) {
      case am.type("TOP_FRECENT_SITES_REQUEST"):
        this._memoized.getTopFrecentSites(msg.data).then(links => {
          let processedLinks = PreviewProvider.preview.processLinks(links);
          PreviewProvider.preview.saveNewLinks(processedLinks);
          const cachedLinks = PreviewProvider.preview.getCachedLinks(processedLinks);
