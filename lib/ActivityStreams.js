...
function ActivityStreams(options = {}) {
  ...
  // This lets us know whether or not this is the first time someone has
  // installed the add-on, so we can wait for previews to come back
  simpleStorage.storage.waitForFirstPreviews = {};
  simpleStorage.storage.waitForFirstPreviews[am.type("TOP_FRECENT_SITES_RESPONSE")] = true;
  simpleStorage.storage.waitForFirstPreviews[am.type("HIGHLIGHTS_LINKS_RESPONSE")] = true;
...
ActivityStreams.prototype = {
  ...
  _processAndSendLinks(links, responseType, append, worker) {
    ...
    const previewsPromise = this._previewProvider.asyncSaveLinks(processedLinks);
    const sendLinks = () => {
      const cachedLinks = this._previewProvider.getEnhancedLinks(processedLinks);
      this.send(am.actions.Response(responseType, cachedLinks, {append}), worker);
    };

    // If this is the first time we load a new tab, we should actually wait for previews
    // to be available before we return a response. Otherwise we should return a
    // response immediately.
    if (simpleStorage.storage.waitForFirstPreviews[responseType]) {
      simpleStorage.storage.waitForFirstPreviews[responseType] = false;
      previewsPromise.then(sendLinks);
    } else {
      sendLinks();
    }
  },
