...
const PREVIEW_CACHE_TIMEOUT = 21600000; // every 6 hours, prime the cache
...
function ActivityStreams(options = {}) {
  ...
  if (this.options.telemetry) {
    if (options.pingEndpoint) {
      this._telemetrySender = new TelemetrySender(options.pingEndpoint);
    }
  ...
  PreviewProvider.preview.cleanUpCache();
...
ActivityStreams.prototype = {
  ...
  _buildPreviewCache(primeCacheTimeout) {
    PlacesProvider.links.getTopFrecentSites().then(links => {
      PreviewProvider.preview.saveNewLinks(links);
    });
    ...
    switch (msgName) {
      case am.type("TOP_FRECENT_SITES_REQUEST"):
        this._memoized.getTopFrecentSites(msg.data).then(links => {
          PreviewProvider.preview.saveNewLinks(links);
          const cachedLinks = PreviewProvider.preview.getCachedLinks(links);
          this.send(am.actions.Response("TOP_FRECENT_SITES_RESPONSE", cachedLinks), worker);
        });
        break;
      case am.type("RECENT_BOOKMARKS_REQUEST"):
        this._memoized.getRecentBookmarks(msg.data).then(links => {
          PreviewProvider.preview.saveNewLinks(links);
          const cachedLinks = PreviewProvider.preview.getCachedLinks(links);
          this.send(am.actions.Response("RECENT_BOOKMARKS_RESPONSE", cachedLinks), worker);
        });
        break;
      case am.type("RECENT_LINKS_REQUEST"):
        this._memoized.getRecentLinks(msg.data).then(links => {
          PreviewProvider.preview.saveNewLinks(links);
          const cachedLinks = PreviewProvider.preview.getCachedLinks(links);
          this.send(am.actions.Response("RECENT_LINKS_RESPONSE", cachedLinks), worker);
        });
        break;
      case am.type("FRECENT_LINKS_REQUEST"):
        this._memoized.getFrecentLinks(msg.data).then(links => {
          PreviewProvider.preview.saveNewLinks(links);
          const cachedLinks = PreviewProvider.preview.getCachedLinks(links);
          this.send(am.actions.Response("FRECENT_LINKS_RESPONSE", cachedLinks), worker);
        });
        break;
  ...
  unload(reason) { // eslint-disable-line no-unused-vars
    switch (reason){
      ...
      default:
        ...
        PreviewProvider.preview.unload();
    }
  }
};
