...
NewTabSearchProvider.prototype = {
  ...
  observe(subject, topic, data) {
    if (topic === CURRENT_ENGINE && data === "engine-current") {
      Task.spawn(function*() {
        let engine = yield this.asyncGetCurrentEngine();
  ...
  asyncGetCurrentState: Task.async(function*() {
    let state = {
      engines: [],
      currentEngine: (yield this.asyncGetCurrentEngine()),
    };
    const pref = Services.prefs.getCharPref(HIDDEN_ENGINES);
    const hiddenEngines = pref ? pref.split(",") : [];
    let result =  Services.search.getVisibleEngines().filter(engine => hiddenEngines.indexOf(engine.name) === -1);
   ...
   */
  asyncGetCurrentEngine: Task.async(function*() {
  ...
  asyncCycleEngine: Task.async(function*(engineName) {
    ...
    const newEngine = yield this.asyncGetCurrentEngine();
