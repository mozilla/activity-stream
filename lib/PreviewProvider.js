...
PreviewProvider.prototype = {
  ...
  _uniqueLinks(links) {
    ...
    links.forEach(site => {
      ...
      site.cacheKey = this._createCacheKey(site.url);
      if (!dedupedSites.has(site.cacheKey)) {
        dedupedSites.set(site.cacheKey, site);
      }
    });
  ...
  saveNewLinks: Task.async(function*(links) {
    let sites = this._uniqueLinks(links.filter(this._URLFilter(URL_FILTERS)));
    let newLinks = sites.filter(link => link && !ss.storage.embedlyData[link.cacheKey]);
    let requestQueue = [];
  ...
  _fetchAndCache: Task.async(function*(newLinks) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = newLinks
            .map(site => {
              const details = json.urls[site.sanitizedURL];
              if (!details) {
                return site;
              }
