...
PreviewProvider.prototype = {
  ...
  _asyncFindItemsInDB: Task.async(function*(links) {
    ...
    links.forEach(link => {
      ...
      const fromCache = MetadataCache.cache.get(key);
      if (fromCache) {
        linksMetadata.push(fromCache);
      } else {
    ...
    if (cacheMissedKeys.length) {
      const fromStore = yield this._metadataStore.asyncGetMetadataByCacheKey(cacheMissedKeys);
      linksMetadata = linksMetadata.concat(fromStore);
    }
