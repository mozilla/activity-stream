...
PreviewProvider.prototype = {
  ...
  getLinkMetadata(links, event = {}, skipPreviewRequest = false, previewsOnly = false) {
    ...
    if (!skipPreviewRequest) {
      this.asyncSaveLinks(processedLinks, event);
    }

    let cachedLinks = this.getEnhancedLinks(processedLinks, previewsOnly, event);
    return this.getFaviconColors(cachedLinks).then(linksToSend => {
      return linksToSend;
    });
  },
  ...
  /**
    ...
    */
  getEnhancedLinks(processedLinks, previewsOnly, event) {
  ...
  asyncBuildCache: Task.async(function*(placesLinks, event = {}) {
    let processedLinks = this.processLinks(placesLinks);
    yield this.asyncSaveLinks(processedLinks, event);
  }),

  /**
   * Request links from embedly, optionally filtering out known links
   */
  asyncSaveLinks: Task.async(function*(processedLinks, event, updateAccessTime = true) {
