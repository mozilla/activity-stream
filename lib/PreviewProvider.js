...
PreviewProvider.prototype = {
  ...
  /**
    * A decodeURIComponent that doesn't throw on invalid URI components
    */
  ...
  _sanitizeURL(url) {
    ...
    if (newURL.search.length !== 0) {
      let queryParams = new Map(newURL.search.slice(1).split("&").map(pair => pair.split("=").map(this._safeDecodeURIComponent)));
