...
PreviewProvider.prototype = {
  ...
  _processLinks(links) {
    return links
      ...
      .map(link => {
        ...
        return Object.assign({}, link, {sanitizedURL, cache_key: cacheKey, places_url: link.url});
      });
  },
  ...
  _asyncGetEnhancedLinks: Task.async(function*(processedLinks, previewsOnly, event) {
    ...
    let DBItems = yield this._asyncFindItemsInDB(processedLinks);
    let itemsMap = new Map();
    DBItems.forEach(item => itemsMap.set(item.cache_key, item));
    let results = processedLinks.map(link => {
      ...
      if (itemsMap.has(link.cache_key)) {
        return Object.assign({}, itemsMap.get(link.cache_key), link);
      } else {
  ...
  _asyncFindItemsInDB: Task.async(function*(links) {
    let cacheKeyArray = [];
    links.forEach(link => cacheKeyArray.push(link.cache_key));
    let items = yield this._metadataStore.asyncGetMetadataByCacheKey(cacheKeyArray);
    return items;
  }),
  /**
  ...
  _asyncSaveLinks: Task.async(function*(processedLinks, event, updateAccessTime = true) {
    ...
    DBItems.forEach(item => setItems.add(item.cache_key));
    let linksList = this._uniqueLinks(processedLinks)
      ...
      .filter(link => !setItems.has(link.cache_key));
   ...
   */
  _asyncFetchAndCache: Task.async(function*(newLinks, event) {
    if (!this.enabled) {
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        for (let link of newLinks) {
          ...
          // Save the entry in the metadata database
          let entry = Object.assign({}, link, {expired_at: (this.options.metadataTTL) + Date.now()});
          yield this._metadataStore.asyncInsert([Object.assign({}, data, entry)]);
        }
      } else {
