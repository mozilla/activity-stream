...
describe("Search", () => {
  ...
  it("should request suggestions input field changes", done => {
    const props = {
      dispatch: (action) => {
  ...
  it("should send perform search event if suggestion is active and enter key is pressed", done => {
    ...
    // suggestion avaible, and the active engine index to -1 so it will use the
  ...
  it("should send remove form history event with proper key binding", done => {
    ...
    setup(props);
    // make sure the drop down is 'visible', set the active suggestion to the first
    // suggestion avaible and press 'Delete'. It will remove the form history entry
  ...
  it("should send cycle current engine event with proper key binding and update current engine", done => {
    const props = {
      dispatch: (action) => {
