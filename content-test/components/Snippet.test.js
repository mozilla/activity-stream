...
describe("Snippet", () => {
  ...
  describe(".visible", () => {
    it("should not be hidden be default", () => {
      assert.notInclude(ReactDOM.findDOMNode(createInstance()).className, "hidden");
    });
    it("should hide the snippet if props.visible is false", () => {
      const instance = createInstance({visible: false});
      assert.include(ReactDOM.findDOMNode(instance).className, "hidden");
    });
  });
  describe(".setVisibility", () => {
    it("should call props.setVisibility when the close button is clicked with false", () => {
