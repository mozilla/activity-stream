...
describe("<TopSite>", () => {
  ...
  it("should have .active class, on top-site-outer if context menu is open", () => {
    const wrapper = shallowWithIntl(
      <TopSite link={link} index={1} activeIndex={1} />
    );
...
describe("<TopSiteForm>", () => {
  ...
  describe("#addMode", () => {
    ...
    it("should have the correct header", () => {
      assert.equal(
        wrapper.findWhere(
          n => n.length && n.props().id === "topsites_form_add_header"
        ).length,
        1
      );
    });
    it("should have the correct button text", () => {
      assert.equal(
        wrapper.findWhere(
          n => n.length && n.props().id === "topsites_form_save_button"
        ).length,
        0
      );
      assert.equal(
        wrapper.findWhere(
          n => n.length && n.props().id === "topsites_form_add_button"
        ).length,
        1
      );
    });
  ...
  describe("edit existing Topsite", () => {
    ...
    it("should have the correct header", () => {
      assert.equal(
        wrapper.findWhere(n => n.props().id === "topsites_form_edit_header")
          .length,
        1
      );
    });
    it("should have the correct button text", () => {
      assert.equal(
        wrapper.findWhere(n => n.props().id === "topsites_form_add_button")
          .length,
        0
      );
      assert.equal(
        wrapper.findWhere(n => n.props().id === "topsites_form_save_button")
          .length,
        1
      );
    });
  ...
  describe("#previewMode", () => {
    ...
    it("should transition from save to preview", () => {
      ...
      assert.equal(
        wrapper.findWhere(
          n => n.length && n.props().id === "topsites_form_preview_button"
        ).length,
        1
      );
    });
...
describe("#TopSiteFormInput", () => {
  ...
  describe("with error", () => {
    ...
    it("should render the error message", () => {
      assert.equal(
        wrapper.findWhere(n => n.props().id === "topsites_form_url_validation")
          .length,
        1
      );
    });
