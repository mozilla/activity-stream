...
describe("<SectionMenu>", () => {
  ...
  it("should show the correct default options for a web extension", () => {
    wrapper = shallowWithIntl(
      <SectionMenu {...DEFAULT_PROPS} isWebExtension={true} />
    );
    const { options } = wrapper.find(ContextMenu).props();
    let i = 0;
    assert.propertyVal(options[i++], "id", "section_menu_action_move_up");
  ...
  it("should show Collapse option for an expanded section if CheckCollapsed in options list", () => {
    wrapper = shallowWithIntl(
      <SectionMenu {...DEFAULT_PROPS} collapsed={false} />
    );
    const { options } = wrapper.find(ContextMenu).props();
    assert.isDefined(
      options.find(o => o.id && o.id === "section_menu_action_collapse_section")
    );
  });
  it("should show Expand option for a collapsed section if CheckCollapsed in options list", () => {
    wrapper = shallowWithIntl(
      <SectionMenu {...DEFAULT_PROPS} collapsed={true} />
    );
    const { options } = wrapper.find(ContextMenu).props();
    assert.isDefined(
      options.find(o => o.id && o.id === "section_menu_action_expand_section")
    );
  });
  ...
  it("should call intl.formatMessage with the correct string ids", () => {
    ...
    assert.ok(
      spy.calledWith(sinon.match({ id: "section_menu_action_move_up" }))
    );
    assert.ok(
      spy.calledWith(sinon.match({ id: "section_menu_action_move_down" }))
    );
    assert.ok(
      spy.calledWith(sinon.match({ id: "section_menu_action_remove_section" }))
    );
    assert.ok(
      spy.calledWith(
        sinon.match({ id: "section_menu_action_collapse_section" })
      )
    );
    assert.ok(
      spy.calledWith(sinon.match({ id: "section_menu_action_manage_section" }))
    );
  });
  it("should disable Move Up on first section", () => {
    wrapper = shallowWithIntl(
      <SectionMenu {...DEFAULT_PROPS} isFirst={true} />
    );
    const { options } = wrapper.find(ContextMenu).props();
