...
exports.test_only_request_links_once = function*(assert) {
  ...
  const endpoint = gPreviewProvider._getCurrentEndpoint(gEmbedlyServiceSource);
...
exports.test_change_metadata_endpoint = function*(assert) {
  ...
  const endpoint = gPreviewProvider._getCurrentEndpoint(gMetadataServiceSource);
  assert.equal(endpoint, `${gEndpointPrefix}${gMetadataServiceEndpoint}${metadataVersionQuery}`, "The new endpoint is set");
  assert.equal(gPreviewProvider._currentSourceName, gMetadataServiceSource, "We updated the source name");
...
exports.test_faulty_metadata_endpoint = function(assert) {
  ...
  let endpoint = gPreviewProvider._getCurrentEndpoint(simplePrefs.prefs.metadataSource);
  const metadataVersionQuery = `?addon_version=${self.version}`;
  ...
  assert.equal(gPreviewProvider._currentSourceName, gEmbedlyServiceSource, "fallback to Embedly as source");

  // directly set the new endpoint
  endpoint = gPreviewProvider._getCurrentEndpoint(gMetadataServiceSource);
  assert.equal(endpoint, `${gEndpointPrefix}${gMetadataServiceEndpoint}${metadataVersionQuery}`, "properly set the endpoint");
  assert.equal(gPreviewProvider._currentSourceName, gMetadataServiceSource, "properly set the source");
};
