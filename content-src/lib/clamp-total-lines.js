...
export function clampTotalLines(parentNode) {
  ...
  toClamp.forEach(child => {
    // Clamp to the remaining allowed or the naturally wrapped line count
    const lines = Math.min(maxLines, child.scrollHeight /
      parseInt(global.getComputedStyle(child).lineHeight, 10));
