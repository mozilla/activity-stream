          ...
          {formHistory.map(suggestion => {
            ...
            return (<li key={suggestion} role="presentation">
                  <a id={"history-search-suggestions-" + suggestionsIdIndex++ }
                     ...
                     aria-selected={active} onClick={() => this.performSearch({engineName: activeEngine.name, searchString: suggestion})}>
        ...
        <ul ref="suggestionsList" role="listbox">
          {suggestions.map(suggestion => {
            ...
            <a id={"search-suggestions-" + suggestionsIdIndex++ }
              ...
              onClick={() => this.performSearch({engineName: activeEngine.name, searchString: suggestion})}>{suggestion}</a>
            </li>);
          })}
        </ul>
      </section>
      ...
      <section className="search-partners" role="group">
            ...
            {engines.map(option => {
              ...
              return (<li key={option.name} className={active ? "active" : ""}>
                <a id={"search-partners-" + enginesIdIndex++ } aria-selected={active}
                      onClick={() => this.performSearch({engineName: option.name, searchString: this.getActiveSuggestion()})}>
                <img src={icon} alt={option.name} width="16px" height="16px"/></a>
              </li>);
            })}
            </ul>
        </section>
