 ...
 * file, You can obtain one at http://mozilla.org/MPL/2.0/. */
 /* globals PlacesProvider, PreviewProvider, Task */
"use strict";
...
this.TopSitesFeed = class TopSitesFeed {
  ...
  getData(action) {
    return Task.spawn(function*() {
      let newAction;
      let links = yield PlacesProvider.links.getLinks();
      let result;
      if (!links) {
        result = [];
      } else {
      ...
      // send an update to content so the preloaded tab can get the udpated content
      this.store.dispatch(ac.SendToContent(newAction, action.meta.fromTarget));
      this.lastUpdated = Date.now();
      for (let link of links) {
        let screenshot = yield PreviewProvider.getThumbnail(link.url);
